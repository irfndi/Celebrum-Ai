...
### Overview
Code analysis is essential for understanding large codebases, identifying refactoring opportunities, and maintaining code quality. This section provides systematic approaches for analyzing code structure, dependencies, and patterns using command-line tools and static analysis.
...
- **Advanced Techniques**:
  - **Export Mapping**: `rg "export \* from" --type ts` to find re-export patterns
  - **Default Exports**: `rg "export default" --type ts` to identify default export usage
  - **Named Exports**: `rg "export \{.*\}" --type ts` for named export patterns
- **Practical Example**:
  ```bash
  # Find all exported functions in services directory
  rg "export (async function|function)" src/services/ --type ts
  
  # Identify barrel exports (re-exports)
...
- **Advanced Techniques**:
  - **Import Graph**: Create dependency maps by combining import/export analysis
  - **Circular Detection**: Track import chains to identify circular dependencies
  - **Unused Imports**: Compare imports against actual usage in files
- **Practical Example**:
  ```bash
  # Find all imports from a specific module
  rg "import.*from ['\"]\.\.?/.*services" src/ --type ts
  
  # Detect potential circular dependencies
  rg "import.*from ['\"]\.\./" src/services/ --type ts | sort
  ```

### Pattern Detection & Code Quality
...
- **Advanced Techniques**:
  - **Duplication Detection**: Search for similar function signatures or patterns
  - **Anti-pattern Identification**: Look for known problematic patterns
  - **Style Consistency**: Check for formatting and naming convention violations
- **Practical Example**:
  ```bash
  # Find all console.log statements (debugging artifacts)
  rg "console\.(log|warn|error|debug)" src/ --type ts
  
  # Identify TODO/FIXME comments for technical debt
  rg "(TODO|FIXME|XXX|HACK):" src/ --type ts -A 1
  
  # Find potential code duplication (similar function names)
  rg "function \w*(create|update|delete)\w*" src/ --type ts
  ```
...
- **Advanced Techniques**:
  - **Type Coverage**: Analyze which types are heavily used vs. rarely used
  - **API Evolution**: Track type changes across versions
  - **Generic Usage**: Understand generic type patterns and constraints
- **Practical Example**:
  ```bash
  # Find all interface definitions
  rg "^export interface \w+" src/ --type ts
  
  # Check usage of specific types
  rg "ServiceContainer|ArbitrageResult" src/ --type ts
  
  # Find generic type usage
  rg "<[A-Z]\w*>" src/ --type ts
  ```
...
- **Advanced Techniques**:
  - **Error Propagation**: Track how errors flow through the application
  - **Error Types**: Catalog different error types and their usage
  - **Recovery Patterns**: Identify retry, fallback, and recovery mechanisms
- **Practical Example**:
  ```bash
  # Find all try-catch blocks
  rg "try \{" src/ --type ts -A 5
  
  # Identify custom error types
  rg "class \w*Error extends|new \w*Error" src/ --type ts
  
  # Check Result type usage patterns
  rg "ArbitrageResult<|Result<" src/ --type ts
  ```

### Performance Analysis Patterns
- **Use Case**: Identifying performance bottlenecks, analyzing async patterns, finding blocking operations
- **Command**: `rg "await|Promise|async" --type ts` for async analysis
- **Scenarios**: Performance optimization, async/await audits, concurrency analysis
- **Advanced Techniques**:
  - **Async Chain Analysis**: Track complex async operation chains
  - **Blocking Operation Detection**: Find synchronous operations in async contexts
  - **Promise Handling**: Analyze Promise creation and resolution patterns
- **Practical Example**:
  ```bash
  # Find all async functions
  rg "async function|async \(" src/ --type ts
  
  # Identify Promise.all usage (parallel operations)
  rg "Promise\.(all|allSettled|race)" src/ --type ts
  
  # Find potential blocking operations
  rg "\.sync\(|setTimeout|setInterval" src/ --type ts
  ```

### Security Pattern Analysis
- **Use Case**: Identifying security vulnerabilities, analyzing authentication flows, checking input validation
- **Command**: `rg "(password|secret|token|auth)" --type ts` for security-sensitive code
- **Scenarios**: Security audits, vulnerability assessment, compliance reviews
- **Advanced Techniques**:
  - **Input Validation**: Check for proper sanitization and validation
  - **Authentication Flow**: Trace auth token handling and validation
  - **Secret Management**: Ensure secrets are properly handled
- **Practical Example**:
  ```bash
  # Find hardcoded secrets or passwords
  rg "(password|secret|key)\s*[:=]\s*['\"][^'\"]{8,}" src/ --type ts
  
  # Check authentication patterns
  rg "(auth|jwt|token|session)" src/ --type ts -i
  
  # Identify input validation patterns
  rg "(validate|sanitize|escape)" src/ --type ts
  ```

### Database & ORM Analysis
- **Use Case**: Understanding database interactions, analyzing query patterns, identifying N+1 problems
- **Command**: `rg "(query|select|insert|update|delete)" --type ts` for database operations
- **Scenarios**: Performance optimization, query analysis, ORM usage review
- **Advanced Techniques**:
  - **Query Pattern Analysis**: Identify common query patterns and optimizations
  - **Transaction Boundaries**: Understand transaction scope and rollback patterns
  - **Connection Management**: Analyze connection pooling and resource management
- **Practical Example**:
  ```bash
  # Find database queries
  rg "(await.*\.(query|findMany|create|update|delete))" src/ --type ts
  
  # Check transaction usage
  rg "(transaction|begin|commit|rollback)" src/ --type ts
  
  # Identify potential N+1 query problems
  rg "for.*await.*\.(find|query)" src/ --type ts
  ```
