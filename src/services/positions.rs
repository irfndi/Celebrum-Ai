...
mod tests {
    ...
    async fn test_create_position_risk_percentage_sizing() {
        ...
        let account_info = AccountInfo { total_balance_usd: 10000.0 };
    ...
    async fn test_set_stop_loss_long_position() {
        ...
        // Create position
        let position = service.create_position(position_data, &account_info).await.unwrap();
    ...
    async fn test_check_risk_triggers_stop_loss() {
        ...
        service.update_position_price(&position.id, 47000.0).await.unwrap();
    ...
    async fn test_get_positions_by_exchange() {
        ...
        let mut position_data1 = create_test_create_position_data(Some(1000.0), None, None, 50000.0);
        position_data1.exchange = ExchangeIdEnum::Binance;
        
        let mut position_data2 = create_test_create_position_data(Some(1000.0), None, None, 50000.0);
        position_data2.exchange = ExchangeIdEnum::Bybit;
        
        service.create_position(position_data1, &account_info).await.unwrap();
        service.create_position(position_data2, &account_info).await.unwrap();
        
        // Get positions by exchange
        let binance_positions = service.get_positions_by_exchange(&ExchangeIdEnum::Binance).await;
        assert!(binance_positions.is_ok());
        assert_eq!(binance_positions.unwrap().len(), 1);
        
        let bybit_positions = service.get_positions_by_exchange(&ExchangeIdEnum::Bybit).await;
    ...
    async fn test_link_positions() {
        ...
        let position1 = service.create_position(
            create_test_create_position_data(Some(1000.0), None, None, 50000.0),
            &account_info
        ).await.unwrap();
        
        let position2 = service.create_position(
            create_test_create_position_data(Some(1000.0), None, None, 50000.0),
            &account_info
        ).await.unwrap();
        
        // Link positions
        let result = service.link_positions(&position1.id, vec![position2.id.clone()]).await;
        assert!(result.is_ok());
        assert!(result.unwrap());
    ...
    async fn test_analyze_position() {
        ...
        // Create position
        let position = service.create_position(
            create_test_create_position_data(Some(1000.0), None, None, 50000.0),
            &account_info
        ).await.unwrap();
        
        // Update price to create PnL
        service.update_position_price(&position.id, 52000.0).await.unwrap();
    ...
    async fn test_validate_position_risk() {
        ...
        // Test valid position
        let position_data = create_test_create_position_data(Some(1000.0), None, None, 50000.0);
        let result = service.validate_position_risk(&position_data, &config).await;
        assert!(result.is_ok());
        
        // Test position size too large
        let large_position_data = create_test_create_position_data(Some(10000.0), None, None, 50000.0);
        let result = service.validate_position_risk(&large_position_data, &config).await;
        assert!(result.is_err());
    }
