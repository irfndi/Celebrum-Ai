...
impl OpportunityCategory {
    ...
    pub fn description(&self) -> &'static str {
        match self {
            OpportunityCategory::LowRiskArbitrage => "Conservative cross-exchange price differences with minimal risk",
            OpportunityCategory::HighConfidenceArbitrage => "High confidence arbitrage opportunities (90%+ accuracy)",
            OpportunityCategory::TechnicalSignals => "Technical analysis based trading signals",
            OpportunityCategory::MomentumTrading => "Opportunities based on price momentum and trends",
            OpportunityCategory::MeanReversion => "Price reversion to mean value strategies",
            OpportunityCategory::BreakoutPatterns => "Price breakout and pattern recognition opportunities",
            OpportunityCategory::HybridEnhanced => "Arbitrage opportunities enhanced with technical analysis",
            OpportunityCategory::AiRecommended => "AI-validated and recommended trading opportunities",
            OpportunityCategory::BeginnerFriendly => "Simple, low-risk opportunities perfect for beginners",
            OpportunityCategory::AdvancedStrategies => "Complex strategies requiring trading experience",
        }
    }
...
impl OpportunityCategorizationService {
    ...
    pub async fn categorize_opportunity(
        ...
        let risk_indicator = RiskIndicator::new(
            opportunity.risk_level.clone(),
        ...
        let (alert_eligible, alert_priority) = self.determine_alert_eligibility(
            &primary_category,
            &opportunity,
            &user_opp_prefs
        );
    ...
    pub async fn filter_opportunities_for_user(
        ...
        for opportunity in opportunities {
            match self.categorize_opportunity(opportunity, user_id).await {
                ...
                Err(e) => {
                    self.logger.warn(&format!("Failed to categorize opportunity: {}", e));
                    continue;
                }
            }
        }
        ...
        categorized_opportunities.sort_by(|a, b| {
            b.user_suitability_score.partial_cmp(&a.user_suitability_score)
                .unwrap_or(std::cmp::Ordering::Equal)
        });
        
        // Apply user's maximum simultaneous opportunities limit
        let user_opp_prefs = self.get_user_opportunity_preferences(user_id).await?;
        let max_count = user_opp_prefs.personalization_settings.max_simultaneous_opportunities as usize;
    ...
    pub async fn update_category_alert_config(
        ...
        if let Some(existing_config) = user_prefs.alert_configs.iter_mut()
    ...
    fn select_primary_category(
        ...
        categories.first().cloned().unwrap_or(OpportunityCategory::BeginnerFriendly)
    }
    ...
    fn calculate_user_suitability(
        ...
        let focus_match = match user_prefs.trading_focus {
            TradingFocus::Arbitrage => {
                matches!(opportunity.opportunity_type, OpportunityType::Arbitrage | OpportunityType::ArbitrageTechnical)
            }
        ...
        let experience_suitable = categories.iter().any(|cat| {
            cat.is_suitable_for_experience(&user_prefs.experience_level)
        });
        ...
        if user_opp_prefs.personalization_settings.preferred_trading_pairs.contains(&opportunity.trading_pair) {
            score += 0.1;
            factors.push("Preferred trading pair".to_string());
        }
        
        // Excluded pair check
        if user_opp_prefs.personalization_settings.excluded_trading_pairs.contains(&opportunity.trading_pair) {
            score -= 0.5;
    ...
    fn determine_alert_eligibility(
        ...
        let alert_config = user_prefs.alert_configs.iter()
        ...
        // Check thresholds
        let meets_confidence = opportunity.confidence_score >= alert_config.min_confidence_threshold;
    ...
    async fn passes_user_filters(
        ...
        let category_enabled = categorized_opp.categories.iter()
        ...
        let excluded_exchange = categorized_opp.base_opportunity.exchanges.iter()
            .any(|exchange| user_opp_prefs.personalization_settings.excluded_exchanges.contains(exchange));
        ...
        if user_opp_prefs.personalization_settings.excluded_trading_pairs
    ...
    fn generate_enhanced_metadata(
        ...
        let category_names: Vec<String> = categories.iter()
            ...
            .collect();
        metadata.insert("category_names".to_string(), serde_json::json!(category_names));
        
        // Add risk assessment details
        metadata.insert("risk_factors".to_string(), serde_json::json!({
        ...
        metadata.insert("recommendations".to_string(), serde_json::json!({
...
mod tests {
    ...
    fn test_opportunity_category_display_names() {
        assert_eq!(OpportunityCategory::LowRiskArbitrage.display_name(), "Low Risk Arbitrage");
        assert_eq!(OpportunityCategory::TechnicalSignals.display_name(), "Technical Signals");
        assert_eq!(OpportunityCategory::BeginnerFriendly.display_name(), "Beginner Friendly");
    }
    
    #[test]
    fn test_category_risk_assessment() {
        assert_eq!(OpportunityCategory::LowRiskArbitrage.risk_assessment(), RiskLevel::Low);
        assert_eq!(OpportunityCategory::MomentumTrading.risk_assessment(), RiskLevel::High);
        assert_eq!(OpportunityCategory::TechnicalSignals.risk_assessment(), RiskLevel::Medium);
    }
