...
impl EnhancedOpportunityService {
    ...
    pub async fn find_enhanced_opportunities(
        ...
        let arbitrage_opportunities = self.find_basic_arbitrage_opportunities(exchange_ids, pairs, threshold).await?;
        ...
        if let Some(ref prefs) = user_preferences {
            if prefs.trading_focus == TradingFocus::Arbitrage && !self.config.enable_technical_analysis {
                ...
                for arb_opp in arbitrage_opportunities {
                    let trading_opp = self.convert_arbitrage_to_trading_opportunity(arb_opp, 1.0, "arbitrage_only").await?;
        ...
        for arb_opportunity in &arbitrage_opportunities {
            match self.enhance_arbitrage_with_technical_analysis(arb_opportunity, user_preferences.as_ref()).await {
    ...
    async fn enhance_arbitrage_with_technical_analysis(
        ...
        user_preferences: Option<&crate::services::user_trading_preferences::UserTradingPreferences>,
    ) -> ArbitrageResult<Option<TradingOpportunity>> {
        let pair = &arbitrage_opportunity.pair;
        
        // Get price data for technical analysis
        let technical_score = self.calculate_technical_confirmation_score(
            pair,
        ...
        let combined_confidence = self.combine_confidence_scores(arbitrage_confidence, technical_score);
        ...
        let enhanced_opportunity = self.convert_arbitrage_to_trading_opportunity(
    ...
    async fn calculate_technical_confirmation_score(
        ...
        for exchange_id in &exchanges_to_analyze {
            if let Some(price_series) = self.market_analysis_service.get_price_series(exchange_id, pair) {
    ...
    async fn calculate_correlation_score(
        ...
        let series1 = self.market_analysis_service.get_price_series(exchange1, pair);
        let series2 = self.market_analysis_service.get_price_series(exchange2, pair);
    ...
    fn passes_user_preference_filter(
        ...
        match preferences.risk_tolerance {
            RiskTolerance::Conservative => {
                technical_score >= 0.7 && combined_confidence >= 0.8
            }
            RiskTolerance::Balanced => {
                technical_score >= 0.5 && combined_confidence >= 0.6
            }
    ...
    async fn convert_arbitrage_to_trading_opportunity(
        ...
        let target_price = arbitrage_opportunity.long_rate.map(|rate| rate * (1.0 + arbitrage_opportunity.rate_difference));
