...
impl AffiliationService {
    ...
    pub async fn review_application(
        ...
        if matches!(decision, VerificationStatus::Approved) {
            self.create_affiliation_program_from_application(application_id, reviewer_id).await?;
        }
    ...
    pub async fn calculate_metrics(&self, user_id: &str, period_days: u32) -> ArbitrageResult<AffiliationMetrics> {
        ...
        let referrals_generated = self.count_referrals_in_period(user_id, &period_start, &period_end).await?;
        let conversions = self.count_conversions_in_period(user_id, &period_start, &period_end).await?;
        let revenue_generated = self.calculate_revenue_in_period(user_id, &period_start, &period_end).await?;
        ...
        let engagement_score = self.calculate_engagement_score(referrals_generated, conversions).await?;
        let performance_tier = self.determine_performance_tier(engagement_score, revenue_generated).await?;
    ...
    async fn create_affiliation_program_from_application(&self, application_id: &str, reviewer_id: &str) -> ArbitrageResult<()> {
        ...
        let kickback_rate = self.calculate_initial_kickback_rate(&application.program_type, application.follower_count);
        ...
        let program = AffiliationProgram {
            ...
            kickback_rate,
            special_features: self.get_default_special_features(&application.program_type),
            created_at: Utc::now(),
            updated_at: Utc::now(),
    ...
    fn calculate_initial_kickback_rate(&self, program_type: &AffiliationProgramType, follower_count: u32) -> f64 {
    ...
    async fn get_application_by_id(&self, application_id: &str) -> ArbitrageResult<AffiliationApplication> {
        ...
        let result = self.d1_service.query(query, &[application_id.into()]).await?;
