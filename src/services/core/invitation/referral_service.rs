...
impl ReferralService {
    ...
    pub async fn use_referral_code(&self, referral_code: &str, new_user_id: &str) -> Result<ReferralUsage> {
        ...
        "#;
        let existing_result = self.d1_service.query(existing_usage_check, &[new_user_id.into()]).await?;
        if let Some(row) = existing_result.first() {
            let count: i32 = row.get("count").unwrap_or("0").parse().unwrap_or(0);
            if count > 0 {
                return Err(anyhow!("User has already been referred and cannot use another referral code"));
            }
        }
        ...
        match self.store_referral_usage(&usage).await {
            Ok(_) => {
                ...
                if let Err(stats_error) = self.update_referrer_statistics(&referrer.user_id).await {
                    ...
                    eprintln!("Warning: Failed to update referrer statistics for user {}: {}", referrer.user_id, stats_error);
                }
                Ok(usage)
            }
            Err(e) => {
                ...
                if error_msg.contains("unique") || error_msg.contains("constraint") {
                    Err(anyhow!("User has already been referred and cannot use another referral code"))
                } else {
                    Err(e)
                }
            }
    ...
    async fn find_user_by_referral_code(&self, referral_code: &str) -> Result<UserReferralCode> {
        ...
        if let Some(row) = result.first() {
            ...
            let total_bonuses_earned = total_bonuses_str.parse::<f64>()
                .map_err(|e| anyhow!("Invalid total_bonuses_earned format '{}': {}", total_bonuses_str, e))?;
    ...
    async fn calculate_total_bonuses_earned(&self, user_id: &str) -> Result<f64> {
        ...
        if let Some(row) = result.first() {
            let total_str = row.get("total")
                .ok_or_else(|| anyhow!("Missing total field in database result"))?;
            let total = total_str.parse::<f64>()
