...
impl CoinMarketCapService {
    ...
    pub async fn get_priority_quotes(&mut self) -> ArbitrageResult<Vec<CmcQuoteData>> {
        ...
        if !self.check_rate_limit().await? {
            self.logger
                .warn("CMC rate limit exceeded, using cached data only");
        ...
        if !self.check_quota_available(1).await? {
            self.logger
                .warn("CMC quota exhausted, using cached data only");
            return self.get_cached_priority_quotes().await;
        }
    ...
    pub async fn get_global_metrics(&mut self) -> ArbitrageResult<CmcGlobalMetrics> {
        ...
        if !self.check_rate_limit().await? {
            self.logger
                .warn("CMC rate limit exceeded, using cached global metrics");
    ...
    pub async fn get_symbol_quote(&self, symbol: &str) -> ArbitrageResult<CmcQuoteData> {
        ...
        // Check rate limit
        if !self.check_rate_limit().await? {
            return Err(ArbitrageError::rate_limit_exceeded(
                "CMC rate limit exceeded",
            ));
        }
    ...
    async fn fetch_quotes_by_symbol(&self, symbols: &str) -> ArbitrageResult<Vec<CmcQuoteData>> {
        ...
        self.logger.info(&format!(
            "CMC API response received for symbols: {}",
            symbols
        ));
    ...
    ) -> ArbitrageResult<Vec<CmcQuoteData>> {
                        ...
                        if let Some(usd_quote) = quote.get("quote").and_then(|q| q.get("USD")) {
                            let quote_obj = CmcQuoteData {
                                ...
                                price: usd_quote
                                    ...
                                    .and_then(|p| p.as_f64())
                                    .unwrap_or(0.0),
                                volume_24h: usd_quote
                                    .get("volume_24h")
                                    .and_then(|v| v.as_f64())
                                    .unwrap_or(0.0),
                                percent_change_1h: usd_quote
                                    .get("percent_change_1h")
                                    .and_then(|p| p.as_f64())
                                    .unwrap_or(0.0),
                                percent_change_24h: usd_quote
                                    .get("percent_change_24h")
                                    .and_then(|p| p.as_f64())
                                    .unwrap_or(0.0),
                                percent_change_7d: usd_quote
                                    .get("percent_change_7d")
                                    .and_then(|p| p.as_f64())
                                    .unwrap_or(0.0),
                                market_cap: usd_quote
                                    .get("market_cap")
                                    .and_then(|m| m.as_f64())
                                    .unwrap_or(0.0),
                                last_updated: usd_quote
                                    .get("last_updated")
                                    .and_then(|u| u.as_str())
                                    .unwrap_or("")
                                    .to_string(),
    ...
    fn parse_global_metrics_response(
        &self,
        response: &serde_json::Value,
    ) -> ArbitrageResult<CmcGlobalMetrics> {
        if let Some(data) = response.get("data") {
            if let Some(quote) = data.get("quote").and_then(|q| q.get("USD")) {
                return Ok(CmcGlobalMetrics {
                    ...
                    total_volume_24h: quote
                        .get("total_volume_24h")
                        .and_then(|v| v.as_f64())
                        .unwrap_or(0.0),
                    bitcoin_dominance: data
                        .get("btc_dominance")
                        .and_then(|b| b.as_f64())
                        .unwrap_or(0.0),
        ...
        Err(ArbitrageError::parsing_error(
            "Failed to parse CMC global metrics",
        ))
    }
    ...
    async fn increment_quota_usage(&self, credits_used: u32) -> ArbitrageResult<()> {
        ...
        self.logger.info(&format!(
            ...
            usage.daily_credits_used,
            self.config.daily_credit_target,
            usage.monthly_credits_used,
        ...
        if let Some(rate_data) = self.kv_store.get(rate_limit_key).text().await? {
            if let Ok(mut status) = serde_json::from_str::<RateLimitStatus>(&rate_data) {
                ...
                status.can_make_request =
                    status.requests_this_minute < self.config.rate_limit_per_minute;
...
// Tests moved to packages/worker/tests/market_data/coinmarketcap_test.rs
