...
mod tests {
    ...
    async fn test_metrics_collection() {
        ...
        let analytics_metrics = service.get_analytics_metrics().await;
        assert_eq!(analytics_metrics.events_processed, 0);

        let cleanup_metrics = service.get_cleanup_metrics().await;
        assert_eq!(cleanup_metrics.total_operations, 0);
    }
}
