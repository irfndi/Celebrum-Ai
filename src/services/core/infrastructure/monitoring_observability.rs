...
impl MonitoringObservabilityService {
    ...
    async fn check_user_permission(&self, user_id: &str, permission: &CommandPermission) -> bool {
        ...
        let telegram_id = match user_id.parse::<i64>() {
            ...
            Ok(_) => {
                crate::utils::logger::logger().warn("Monitoring access denied: Invalid user ID format (non-positive)");
                return false;
            }
            Err(_) => {
                crate::utils::logger::logger().warn("Monitoring access denied: Invalid user ID format (parse error)");
                return false;
            }
        };
            ...
            .await
        {
            ...
            Ok(None) => {
                crate::utils::logger::logger().warn("Monitoring access denied: User not found in database");
                return false;
            }
            Err(_) => {
                crate::utils::logger::logger().warn("Monitoring access denied: Database error during user lookup");
                return false;
            }
        };
    ...
    pub async fn record_performance(
        ...
        if should_update_percentiles {
            ...
            let mut perf_metrics = match self.performance_metrics.write() {
                Ok(guard) => guard,
                Err(_) => {
                    crate::utils::logger::logger().error("Failed to acquire write lock on performance metrics");
                    return Ok(());
                }
            };
    ...
    async fn check_alert_conditions(&self, metric: &MetricDataPoint) -> ArbitrageResult<()> {
        let rules = {
            let rules_guard = match self.alert_rules.read() {
                Ok(guard) => guard,
                Err(_) => {
                    crate::utils::logger::logger().error("Failed to acquire read lock on alert rules");
                    return Ok(());
                }
            };
            rules_guard.clone()
            ...
            if should_alert {
                ...
                let recent_alert = {
                    let active_alerts = match self.active_alerts.read() {
                        Ok(guard) => guard,
                        Err(_) => {
                            crate::utils::logger::logger().error("Failed to acquire read lock on active alerts");
                            return Ok(());
                        }
                    };
    ...
    async fn trigger_alert(&self, rule: &AlertRule, current_value: f64) -> ArbitrageResult<()> {
...

        {
            let mut alerts = match self.active_alerts.write() {
                Ok(guard) => guard,
                Err(_) => {
                    crate::utils::logger::logger().error("Failed to acquire write lock on active alerts");
                    return Ok(());
                }
            };
    ...
    async fn get_real_cpu_usage(&self) -> f64 {
        let mut system = match self.system.write() {
            Ok(guard) => guard,
            Err(_) => {
                crate::utils::logger::logger().error("Failed to acquire write lock on system monitor for CPU");
                return 0.0;
            }
        };
    ...
    async fn get_real_memory_usage(&self) -> f64 {
        let mut system = match self.system.write() {
            Ok(guard) => guard,
            Err(_) => {
                crate::utils::logger::logger().error("Failed to acquire write lock on system monitor for memory");
                return 0.0;
            }
        };
    ...
    async fn get_real_disk_usage(&self) -> f64 {
        let mut system = match self.system.write() {
            Ok(guard) => guard,
            Err(_) => {
                crate::utils::logger::logger().error("Failed to acquire write lock on system monitor for disk");
    ...
    async fn check_service_health(&self, service_name: &str) -> bool {
        ...
        match service_name {
            ...
            _ => {
                crate::utils::logger::logger().warn(&format!("Unknown service for health check: {}", service_name));
                false
            }
        }
    }
