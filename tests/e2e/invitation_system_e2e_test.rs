...
use arb_edge::types::{UserProfile, SubscriptionTier, CommandPermission, UserRole};
...
impl InvitationTestEnvironment {
    async fn new() -> Self {
        ...
        let d1_service = D1Service::new(mock_env.clone()).await.expect("Failed to create D1Service");
    ...
    async fn generate_invitation_code(&mut self, admin_user_id: &str) -> ArbitrageResult<String> {
        let code = self.invitation_service.generate_invitation_code(admin_user_id, None, None).await?;
        self.generated_codes.push(code.clone());
        Ok(code)
    }

    async fn register_user_with_invitation(&mut self, telegram_id: i64, invitation_code: &str) -> ArbitrageResult<UserProfile> {
        // Validate invitation code and create user
        let usage = self.invitation_service.use_invitation_code(invitation_code, telegram_id).await?;
        ...
        // Store user
        self.user_profile_service.create_user_profile(user.clone()).await?;
        self.registered_users.insert(user.user_id.clone(), user.clone());
...
mod invitation_system_e2e_tests {
    ...
    async fn test_super_admin_invitation_code_generation() {
        ...
        for (i, code) in generated_codes.iter().enumerate() {
            ...
            assert!(code.len() >= 8, "Code {} should be at least 8 characters", i + 1);
        ...
        assert_eq!(unique_codes.len(), generated_codes.len(), "All codes should be unique");
        println!("   Uniqueness: All {} codes are unique ✅", generated_codes.len());
    ...
    async fn test_user_registration_with_invitation_code() {
        ...
        let invitation_code = test_env.generate_invitation_code(super_admin_id).await
        ...
        let user = test_env.register_user_with_invitation(telegram_id, &invitation_code).await
        ...
        assert!(user.has_active_beta_access(), "User should have active beta access");
        ...
        for (permission, name) in &beta_permissions {
            ...
            println!("   {}: {} ✅", name, if has_permission { "GRANTED" } else { "DENIED" });
        }
        ...
        for (permission, name) in &admin_permissions {
            let has_permission = test_env.check_user_permission(&user.user_id, permission.clone());
            assert!(!has_permission, "Beta user should NOT have {} permission", name);
            println!("   {}: {} ✅", name, if has_permission { "GRANTED" } else { "CORRECTLY DENIED" });
        }
    ...
    async fn test_beta_expiration_and_auto_downgrade() {
        ...
        let mut user = test_env.register_user_with_invitation(987654321, &invitation_code).await
        ...
        test_env.registered_users.insert(user.user_id.clone(), user.clone());
        ...
        assert!(!user.has_active_beta_access(), "User should not have active beta access");
        println!("   Beta Status: Expired ✅");
        
        // **Step 3: Check that user needs downgrade**
        assert!(user.needs_beta_downgrade(), "User should need beta downgrade");
        ...
        for (permission, name) in &premium_permissions {
            ...
            assert!(!has_permission, "Expired beta user should NOT have {} permission", name);
            println!("   {}: {} ✅", name, if has_permission { "GRANTED" } else { "CORRECTLY DENIED" });
        }
    ...
    async fn test_invalid_invitation_code_handling() {
        ...
        let result = test_env.register_user_with_invitation(111111111, invalid_code).await;
        assert!(result.is_err(), "Registration with invalid code should fail");
        println!("   Invalid code rejection: ✅");
        
        // **Step 2: Test empty code**
        println!("\n🔍 Testing empty invitation code...");
        let empty_code = "";
        let result = test_env.register_user_with_invitation(222222222, empty_code).await;
        ...
        let valid_code = test_env.generate_invitation_code(super_admin_id).await
            .expect("Failed to generate invitation code");
        
        // Use the code once
        let _first_user = test_env.register_user_with_invitation(333333333, &valid_code).await
            .expect("First registration should succeed");
        println!("   First use: Success ✅");
        
        // Try to use the same code again
        let result = test_env.register_user_with_invitation(444444444, &valid_code).await;
        assert!(result.is_err(), "Registration with already used code should fail");
    ...
    async fn test_invitation_system_statistics() {
        ...
        for i in 1..=3 {
            let code = test_env.generate_invitation_code(super_admin_id).await
                .expect("Failed to generate code");
            codes.push(code.clone());
            
            let user = test_env.register_user_with_invitation(1000000000 + i, &code).await
                .expect("Failed to register user");
            users.push(user);
        ...
        assert_eq!(test_env.generated_codes.len(), 3, "Should have 3 generated codes");
        assert_eq!(test_env.registered_users.len(), 3, "Should have 3 registered users");
        
        println!("   Total codes generated: {} ✅", test_env.generated_codes.len());
        println!("   Total users registered: {} ✅", test_env.registered_users.len());
        
        // **Step 3: Validate all users have beta access**
        println!("\n🧪 Validating beta access for all users...");
        for (i, user) in users.iter().enumerate() {
            assert!(user.has_active_beta_access(), "User {} should have beta access", i + 1);
