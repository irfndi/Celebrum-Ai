...
// NOTE: These tests are disabled due to missing service constructors and complex dependencies
...
mod disabled_tests {
    ...
    async fn test_rsi_confidence_calculation() {
        ...
        let very_oversold_confidence =
            service.calculate_rsi_confidence(15.0, TradingSignalType::Buy);
        let strongly_oversold_confidence =
            service.calculate_rsi_confidence(25.0, TradingSignalType::Buy);
        let moderately_oversold_confidence =
            service.calculate_rsi_confidence(30.0, TradingSignalType::Buy);
        ...
        let very_overbought_confidence =
            service.calculate_rsi_confidence(85.0, TradingSignalType::Sell);
        let strongly_overbought_confidence =
            service.calculate_rsi_confidence(75.0, TradingSignalType::Sell);
        let moderately_overbought_confidence =
            service.calculate_rsi_confidence(70.0, TradingSignalType::Sell);
    ...
    async fn test_crossover_confidence_calculation() {
        ...
        let strong_crossover =
            service.calculate_crossover_confidence(102.0, 100.0, TradingSignalType::Buy);
        assert!((strong_crossover - 0.8).abs() < 1e-10);

        // Test moderate crossover (>1% difference)
        let moderate_crossover =
            service.calculate_crossover_confidence(101.0, 100.0, TradingSignalType::Buy);
        assert!((moderate_crossover - 0.7).abs() < 1e-10);

        // Test weak crossover (<1% difference)
        let weak_crossover =
            service.calculate_crossover_confidence(100.5, 100.0, TradingSignalType::Buy);
    ...
    async fn test_bollinger_confidence_calculation() {
        ...
        let strong_buy_signal =
            service.calculate_bollinger_confidence(90.0, 95.0, 105.0, TradingSignalType::Buy);
        let weak_buy_signal =
            service.calculate_bollinger_confidence(94.0, 95.0, 105.0, TradingSignalType::Buy);
        ...
        let strong_sell_signal =
            service.calculate_bollinger_confidence(110.0, 95.0, 105.0, TradingSignalType::Sell);
        let weak_sell_signal =
            service.calculate_bollinger_confidence(106.0, 95.0, 105.0, TradingSignalType::Sell);
    ...
    async fn test_price_targets_calculation() {
        ...
        let (buy_target, buy_stop_loss) =
            service.calculate_price_targets(entry_price, &TradingSignalType::Buy);
        ...
        let (sell_target, sell_stop_loss) =
            service.calculate_price_targets(entry_price, &TradingSignalType::Sell);
        ...
        let (hold_target, hold_stop_loss) =
            service.calculate_price_targets(entry_price, &TradingSignalType::Hold);
    ...
    async fn test_user_preference_filtering() {
        ...
        let beginner_prefs =
            create_test_user_preferences(ExperienceLevel::Beginner, RiskTolerance::Conservative);
        ...
        let advanced_prefs =
            create_test_user_preferences(ExperienceLevel::Advanced, RiskTolerance::Aggressive);
    ...
    async fn test_signal_to_opportunity_conversion() {
        ...
        let trading_opportunity = service
            .convert_signal_to_opportunity(test_signal.clone())
            .await
            .unwrap();

        assert_eq!(trading_opportunity.opportunity_id, test_signal.signal_id);
        assert_eq!(
            trading_opportunity.opportunity_type,
        ...
        assert!(
            (trading_opportunity.confidence_score - test_signal.confidence_score).abs() < 1e-10
        );
        ...
        assert_eq!(
            trading_opportunity.indicators_used,
            vec![test_signal.indicator_source]
        );
    ...
    async fn test_risk_level_assignment() {
        ...
        let very_strong_opp = service
            .convert_signal_to_opportunity(very_strong_signal)
            .await
            .unwrap();
        ...
        let strong_opp = service
            .convert_signal_to_opportunity(strong_signal)
            .await
            .unwrap();
        assert_eq!(strong_opp.risk_level, RiskLevel::Medium);

        // Test Moderate -> Medium risk
        let moderate_opp = service
            .convert_signal_to_opportunity(base_signal.clone())
            .await
            .unwrap();
        ...
        let weak_opp = service
            .convert_signal_to_opportunity(weak_signal)
            .await
            .unwrap();
    ...
    async fn test_time_horizon_assignment() {
        ...
        let rsi_opp = service
            .convert_signal_to_opportunity(rsi_signal)
            .await
            .unwrap();
        ...
        let ma_opp = service
            .convert_signal_to_opportunity(ma_signal)
            .await
            .unwrap();
        ...
        let bb_opp = service
            .convert_signal_to_opportunity(bb_signal)
            .await
            .unwrap();
        ...
        let momentum_opp = service
            .convert_signal_to_opportunity(momentum_signal)
            .await
            .unwrap();
    ...
    async fn test_find_technical_opportunities_empty_result() {
        ...
        let opportunities = service
            .find_technical_opportunities(&exchange_ids, &pairs, None)
            .await
            .unwrap();
    ...
    async fn test_find_technical_opportunities_arbitrage_only_user() {
        ...
        let opportunities = service
            .find_technical_opportunities(&exchange_ids, &pairs, Some("arbitrage_only_user"))
            .await
            .unwrap();
