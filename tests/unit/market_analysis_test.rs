...
use arbedge::services::market_analysis::*;
...
async fn test_bollinger_bands() {
    ...
    for i in 0..upper.len() {
        assert!(upper[i] > middle[i], "Upper band should be above middle at index {}", i);
        assert!(lower[i] < middle[i], "Lower band should be below middle at index {}", i);
    }
...
async fn test_price_correlation() {
    ...
    assert!((correlation - 1.0).abs() < 0.001, "Expected perfect positive correlation, got {}", correlation);
    ...
    assert!((correlation_negative + 1.0).abs() < 0.001, "Expected perfect negative correlation, got {}", correlation_negative);
...
async fn test_mathematical_edge_cases() {
    ...
    let minimal_prices = vec![
        ...
        10.0, 11.0, 12.0, 13.0, 14.0, 13.0  // 16 prices for RSI 14
    ];
