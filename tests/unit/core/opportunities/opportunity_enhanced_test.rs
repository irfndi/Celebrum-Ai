...
// NOTE: These tests are disabled due to private method access and complex mock setup requirements
...
mod disabled_tests {
    ...
    use arb_edge::services::core::opportunities::opportunity_enhanced::{
        EnhancedOpportunityService, EnhancedOpportunityServiceConfig,
    };
    use arb_edge::services::core::trading::exchange::ExchangeService;
    ...
    fn create_test_config() -> EnhancedOpportunityServiceConfig {
        EnhancedOpportunityServiceConfig {
            ...
            monitored_pairs: vec![StructuredTradingPair {
    ...
    async fn test_volatility_score_calculation() {
        ...
        let stable_prices = vec![
            100.0, 100.1, 99.9, 100.0, 100.2, 99.8, 100.1, 99.9, 100.0, 100.1,
        ];
        ...
        let volatile_prices = vec![
            100.0, 110.0, 90.0, 105.0, 85.0, 115.0, 95.0, 120.0, 80.0, 125.0,
        ];
        let volatile_series = create_test_price_series(volatile_prices);
        let volatile_score = service
            .calculate_volatility_score(&volatile_series)
            .unwrap();
    ...
    async fn test_trading_opportunity_conversion() {
        ...
        assert!(trading_opportunity
            .indicators_used
            .contains(&"funding_rate".to_string()));
        assert!(trading_opportunity
            .indicators_used
    ...
    async fn test_risk_level_assignment() {
        ...
        let high_conf_opp = service
            .convert_arbitrage_to_trading_opportunity(base_opportunity.clone(), 0.85, "test")
            .await
            .unwrap();
        assert_eq!(high_conf_opp.risk_level, RiskLevel::Low);

        // Medium confidence -> Medium risk
        let med_conf_opp = service
            .convert_arbitrage_to_trading_opportunity(base_opportunity.clone(), 0.65, "test")
            .await
            .unwrap();
        assert_eq!(med_conf_opp.risk_level, RiskLevel::Medium);

        // Low confidence -> High risk
        let low_conf_opp = service
            .convert_arbitrage_to_trading_opportunity(base_opportunity.clone(), 0.45, "test")
            .await
            .unwrap();
    ...
    async fn test_enhanced_opportunity_detection_empty_arbitrage() {
        ...
        let opportunities = service
            .find_enhanced_opportunities(&exchange_ids, &pairs, threshold, None)
            .await
            .unwrap();
    ...
    fn create_test_arbitrage_opportunity(
        rate_difference: f64,
