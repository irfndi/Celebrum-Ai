...
use arb_edge::types::{
    ExchangeIdEnum, InvitationCode, UserApiKey, UserProfile, UserSession, SessionState, ApiKeyProvider
};
...
impl MockD1Service {
    ...
    async fn mock_get_user_by_telegram_id(&self, telegram_id: i64) -> ArbitrageResult<Option<UserProfile>> {
        if let Some(ref error_type) = self.error_simulation {
            return match error_type.as_str() {
                "database_error" => Err(ArbitrageError::database_error("Database connection failed")),
                _ => Err(ArbitrageError::validation_error("Unknown database error")),
    ...
    async fn mock_update_user_profile(&mut self, profile: &UserProfile) -> ArbitrageResult<()> {
        if let Some(ref error_type) = self.error_simulation {
            return match error_type.as_str() {
                "update_failed" => Err(ArbitrageError::database_error("Failed to update user profile")),
    ...
    async fn mock_store_user_api_key(&mut self, user_id: &str, api_key: &UserApiKey) -> ArbitrageResult<()> {
        if let Some(ref error_type) = self.error_simulation {
            return match error_type.as_str() {
                "api_key_storage_failed" => Err(ArbitrageError::database_error("Failed to store API key")),
    ...
    async fn mock_get_invitation_code(&self, code: &str) -> ArbitrageResult<Option<InvitationCode>> {
        if let Some(ref error_type) = self.error_simulation {
            return match error_type.as_str() {
                "invitation_lookup_failed" => Err(ArbitrageError::database_error("Failed to lookup invitation code")),
    ...
    async fn mock_update_invitation_code(&mut self, invitation: &InvitationCode) -> ArbitrageResult<()> {
        if let Some(ref error_type) = self.error_simulation {
            return match error_type.as_str() {
                "invitation_update_failed" => Err(ArbitrageError::database_error("Failed to update invitation code")),
                _ => Err(ArbitrageError::validation_error("Unknown invitation update error")),
            };
        }

        self.invitation_codes.insert(invitation.code.clone(), invitation.clone());
        Ok(())
    }
    ...
    fn add_mock_invitation_code(&mut self, invitation: InvitationCode) {
        self.invitation_codes.insert(invitation.code.clone(), invitation);
    }
    ...
    fn get_api_key_count(&self, user_id: &str) -> usize {
        self.api_keys.get(user_id).map(|keys| keys.len()).unwrap_or(0)
    }
}
...
impl MockKvStore {
    ...
    async fn mock_delete(&mut self, key: &str) -> ArbitrageResult<()> {
        if let Some(ref error_type) = self.error_simulation {
            return match error_type.as_str() {
                "kv_delete_failed" => Err(ArbitrageError::database_error("KV delete operation failed")),
                _ => Err(ArbitrageError::validation_error("Unknown KV error")),
            };
        }
...
impl MockUserProfileService {
    ...
    async fn mock_create_user_profile(
        ...
        if let Some(_existing) = self.d1_service.mock_get_user_by_telegram_id(telegram_user_id).await? {
            return Err(ArbitrageError::validation_error(
    ...
    async fn mock_get_user_by_telegram_id(&mut self, telegram_user_id: i64) -> ArbitrageResult<Option<UserProfile>> {
        ...
        if let Ok(Some(cached_user_id)) = self.kv_store.mock_get(&cache_key).await {
            if let Some(profile) = self.d1_service.mock_get_user_profile(&cached_user_id).await? {
                return Ok(Some(profile));
            }
        }

        // Get from D1 and cache the result
        if let Some(profile) = self.d1_service.mock_get_user_by_telegram_id(telegram_user_id).await? {
            self.mock_cache_telegram_user_mapping(telegram_user_id, &profile.user_id).await?;
            Ok(Some(profile))
        } else {
            Ok(None)
        }
    ...
    async fn mock_update_user_profile(&mut self, profile: &UserProfile) -> ArbitrageResult<()> {
        ...
        self.d1_service.mock_update_user_profile(&updated_profile).await?;
        self.mock_invalidate_user_cache(&updated_profile.user_id).await?;
    ...
    async fn mock_add_user_api_key(
        ...
        self.d1_service.mock_store_user_api_key(user_id, &user_api_key).await?;
    ...
    async fn mock_get_user_session(&self, telegram_chat_id: i64) -> ArbitrageResult<Option<UserSession>> {
    ...
    async fn mock_validate_and_use_invitation_code(&mut self, code: &str) -> ArbitrageResult<()> {
        let mut invitation = self.d1_service.mock_get_invitation_code(code).await?
            .ok_or_else(|| ArbitrageError::not_found("Invitation code not found"))?;

        if !invitation.can_be_used() {
            return Err(ArbitrageError::validation_error("Invitation code cannot be used"));
        }

        if !invitation.use_code() {
            return Err(ArbitrageError::validation_error("Failed to use invitation code"));
        }

        self.d1_service.mock_update_invitation_code(&invitation).await?;
        Ok(())
    }
    ...
    async fn mock_cache_telegram_user_mapping(&mut self, telegram_user_id: i64, user_id: &str) -> ArbitrageResult<()> {
        let cache_key = format!("user_cache:telegram:{}", telegram_user_id);
    ...
    async fn mock_store_invitation_code(&mut self, invitation: &InvitationCode) -> ArbitrageResult<()> {
        // Store in D1
        self.d1_service.mock_update_invitation_code(invitation).await?;
...
mod tests {
    ...
    async fn test_user_profile_creation_and_validation() {
        ...
        let result = service.mock_create_user_profile(telegram_id, invitation_code.clone(), username.clone()).await;
        assert!(result.is_ok());
        ...
        assert!(service.kv_store.contains_key(&format!("user_session:{}", telegram_id)));

        // Test duplicate user creation (should fail)
        let duplicate_result = service.mock_create_user_profile(telegram_id, None, None).await;
    ...
    async fn test_user_profile_validation_errors() {
        ...
        assert!(invalid_result.unwrap_err().to_string().contains("must be positive"));

        // Test invalid telegram ID (zero)
        let zero_result = service.mock_create_user_profile(0, None, None).await;
        assert!(zero_result.is_err());
        assert!(zero_result.unwrap_err().to_string().contains("must be positive"));

        // Test invalid invitation code
        let invalid_invite_result = service.mock_create_user_profile(123456789, Some("INVALID-CODE".to_string()), None).await;
        assert!(invalid_invite_result.is_err());
        assert!(invalid_invite_result.unwrap_err().to_string().contains("not found"));
    }
    ...
    async fn test_user_profile_retrieval_and_caching() {
        ...
        let retrieved_by_telegram = service.mock_get_user_by_telegram_id(telegram_id).await.unwrap();
        assert!(retrieved_by_telegram.is_some());
        assert_eq!(retrieved_by_telegram.unwrap().telegram_user_id, Some(telegram_id));
        ...
        let cached_retrieval = service.mock_get_user_by_telegram_id(telegram_id).await.unwrap();
        assert!(cached_retrieval.is_some());

        // Test non-existent user
        let non_existent = service.mock_get_user_profile("non_existent_user").await.unwrap();
    ...
    async fn test_user_profile_updates() {
        ...
        let updated_profile = service.mock_get_user_profile(&user_id).await.unwrap().unwrap();
        ...
        let non_existent_result = service.mock_update_user_profile(&non_existent_profile).await;
        assert!(non_existent_result.is_err());
        assert!(non_existent_result.unwrap_err().to_string().contains("not found"));
    }
    ...
    async fn test_api_key_management() {
        ...
        let add_result = service.mock_add_user_api_key(
            &user_id,
        ...
        assert_eq!(api_keys[0].provider, ApiKeyProvider::Exchange(ExchangeIdEnum::Binance));
        assert_eq!(api_keys[0].permissions, vec!["read".to_string(), "trade".to_string()]);

        // Test adding second API key for different exchange
        let add_result2 = service.mock_add_user_api_key(
            &user_id,
        ...
        let remove_result = service.mock_remove_user_api_key(&user_id, &ExchangeIdEnum::Binance).await;
        ...
        assert_eq!(api_keys_final[0].provider, ApiKeyProvider::Exchange(ExchangeIdEnum::Bybit));

        // Test removing non-existent API key
        let remove_non_existent = service.mock_remove_user_api_key(&user_id, &ExchangeIdEnum::OKX).await;
    ...
    async fn test_api_key_encryption_and_decryption() {
        ...
        // Decrypt
        let (decrypted_key, decrypted_secret) = service.mock_decrypt_user_api_key(&encrypted_key, &encrypted_secret).await.unwrap();
    ...
    async fn test_session_management() {
        ...
        // Retrieve session
        let retrieved_session = service.mock_get_user_session(telegram_chat_id).await.unwrap();
        ...
        let deleted_session = service.mock_get_user_session(telegram_chat_id).await.unwrap();
    ...
    async fn test_invitation_code_system() {
        ...
        let invitation_result = service.mock_create_invitation_code(
            purpose.clone(),
            max_uses,
            expires_in_days,
            created_by.clone(),
        ...
        let retrieved_invitation = service.mock_get_invitation_code(&invitation.code).await.unwrap();
        assert!(retrieved_invitation.is_some());
        assert_eq!(retrieved_invitation.unwrap().code, invitation.code);

        // Test using invitation code
        let use_result = service.mock_validate_and_use_invitation_code(&invitation.code).await;
        assert!(use_result.is_ok());

        // Verify invitation code usage was tracked
        let used_invitation = service.d1_service.mock_get_invitation_code(&invitation.code).await.unwrap().unwrap();
        ...
        for _ in 0..4 {
            let use_again = service.mock_validate_and_use_invitation_code(&invitation.code).await;
            assert!(use_again.is_ok());
        }

        // Verify invitation code is now exhausted
        let exhausted_invitation = service.d1_service.mock_get_invitation_code(&invitation.code).await.unwrap().unwrap();
        ...
        let exhausted_use = service.mock_validate_and_use_invitation_code(&invitation.code).await;
        assert!(exhausted_use.is_err());
        assert!(exhausted_use.unwrap_err().to_string().contains("cannot be used"));
    }
    ...
    async fn test_error_handling_and_recovery() {
        ...
        assert!(db_error_result.unwrap_err().to_string().contains("Database connection failed"));
        ...
        let api_key_error = service.mock_add_user_api_key(
            &profile.user_id,
            ExchangeIdEnum::Binance,
            "test_key",
            "test_secret",
        ...
        assert!(error_msg.contains("Failed to store API key") || error_msg.contains("Unknown database error"));
    }
    ...
    async fn test_user_profile_business_logic() {
        ...
        let profile_result = service.mock_create_user_profile(
            123456789,
        ...
        let used_invitation = service.d1_service.mock_get_invitation_code(&invitation_code).await.unwrap().unwrap();
        assert_eq!(used_invitation.current_uses, 1);
        assert!(!used_invitation.can_be_used());

        // Add multiple API keys to test minimum exchange requirement
        service.mock_add_user_api_key(
            &user_id,
            ExchangeIdEnum::Binance,
            "binance_key",
            "binance_secret",
        ...
        let profile_after_one_key = service.mock_get_user_profile(&user_id).await.unwrap().unwrap();
        ...
        let profile_after_two_keys = service.mock_get_user_profile(&user_id).await.unwrap().unwrap();
    ...
    async fn test_concurrent_operations_simulation() {
        ...
        for i in 0..5 {
            ...
            let profile_result = service.mock_create_user_profile(telegram_id, None, username).await;
            assert!(profile_result.is_ok());
        ...
        for (i, profile) in user_profiles.iter().enumerate() {
            let exchange = if i % 2 == 0 { ExchangeIdEnum::Binance } else { ExchangeIdEnum::Bybit };
            
            let api_key_result = service.mock_add_user_api_key(
                &profile.user_id,
                exchange,
                &format!("api_key_{}", i),
                &format!("secret_{}", i),
        ...
        for profile in &user_profiles {
            let api_keys = service.mock_get_user_api_keys(&profile.user_id).await.unwrap();
            assert_eq!(api_keys.len(), 1);
        }
