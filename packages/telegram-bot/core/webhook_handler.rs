...
use crate::core::command_router::{CommandContext, CommandRouter, UserPermissions};
...
impl WebhookHandler {
    ...
    async fn handle_text_message(
        ...
        if text.starts_with('/') {
            return self.handle_command(_chat_id, user_id, text).await;
        }

        // Handle regular text
        self.handle_regular_text(_chat_id, user_id, text).await
    }
    ...
    async fn handle_command(
        ...
        if command.is_empty() {
            console_log!(
                "❌ Empty command received from user {} in chat {}",
                user_id,
                chat_id
            );
            return Ok(
                "❌ Invalid command. Please use /help to see available commands.".to_string(),
            );
        }

        if command.len() > 256 {
            console_log!(
                "❌ Command too long ({} chars) from user {} in chat {}",
                command.len(),
                user_id,
                chat_id
            );
        ...
        console_log!(
            "📝 Parsed command: '{}' with {} args from user {}",
            cmd,
            args.len(),
            user_id
        );
        ...
        let user_permissions = match self.get_user_permissions(user_id).await {
            ...
            Err(e) => {
                console_log!(
                    "⚠️ Failed to get user permissions for user {}: {:?}, using defaults",
                    user_id,
                    e
                );
                UserPermissions {
            ...
            .await
        {
            ...
            Err(e) => {
                console_log!(
                    "❌ Command '{}' failed for user {} in chat {}: {:?}",
                    cmd,
                    user_id,
                    chat_id,
                    e
                ...
                match e {
                    ...
                    TelegramError::Http(_) => {
                        Ok("❌ Network error occurred. Please try again later.".to_string())
                    }
                    TelegramError::Json(_) => {
                        Ok("❌ Data processing error. Please try again.".to_string())
                    }
                    TelegramError::Timeout => {
                        Ok("❌ Command timed out. Please try again.".to_string())
                    }
                    TelegramError::RateLimit => {
                        Ok("❌ Too many requests. Please wait a moment and try again.".to_string())
                    }
                }
            }
        }
    ...
    async fn handle_regular_text(
        ...
        if text.is_empty() {
            console_log!(
                "⚠️ Empty text message from user {} in chat {}",
                user_id,
                chat_id
            );
        ...
        if text.len() > 4096 {
            console_log!(
                "⚠️ Text message too long ({} chars) from user {} in chat {}",
                text.len(),
                user_id,
                chat_id
            );
        ...
        console_log!(
            "📝 Processing regular text ({} chars) from user {}",
            text.len(),
            user_id
        );
        ...
        } else if text.to_lowercase().contains("price") || text.to_lowercase().contains("arbitrage")
        {
            ...
            format!(
                "I understand you said: \"{}\". Use /help to see what I can do for you!",
                text
            )
        };

        console_log!(
            "✅ Regular text processed successfully for user {}",
            user_id
        );
        Ok(response)
    }
    ...
    async fn handle_callback_query(&self, callback_query: &Value) -> TelegramResult<String> {
        ...
        let query_id = callback_query
            ...
            .ok_or_else(|| {
                console_log!(
                    "❌ Missing callback query ID in payload: {:?}",
                    callback_query
                );
        ...
        let user_id = callback_query
            ...
            .ok_or_else(|| {
                console_log!(
                    "❌ Missing or invalid user ID in callback query: {:?}",
                    callback_query
                );
        ...
        console_log!(
            "🔘 Callback query: '{}' from user {} in chat {} (query_id: {})",
            data,
            user_id,
            chat_id,
            query_id
        ...
        if data.len() > 64 {
            console_log!(
                "⚠️ Callback data too long ({} chars) from user {}",
                data.len(),
                user_id
            );
        ...
        match self.process_callback_data(data, user_id, chat_id).await {
            Ok(response) => {
                console_log!(
                    "✅ Callback query '{}' processed successfully for user {}",
                    data,
                    user_id
                );
                Ok(response)
            ...
            Err(e) => {
                console_log!(
                    "❌ Callback query '{}' failed for user {}: {:?}",
                    data,
                    user_id,
                    e
                );
    ...
    async fn process_callback_data(
        ...
        console_log!(
            "🔄 Processing callback data: '{}' for user {}",
            data,
            user_id
        );
        ...
        match action {
            ...
            _ => {
                console_log!(
                    "❓ Unknown callback action: '{}' from user {}",
                    action,
                    user_id
                );
