...
async fn test_health_check_registration() {
    ...
    monitor.register_check("database", Box::new(|| async { Ok(HealthStatus::Healthy) }));

    monitor.register_check("redis", Box::new(|| async { Ok(HealthStatus::Healthy) }));
...
async fn test_health_check_failure() {
    ...
    monitor.register_check(
        "failing_service",
        Box::new(|| async { Err("Service unavailable".to_string()) }),
    );
...
async fn test_alert_threshold_monitoring() {
    ...
    let alerts = monitor
        .check_thresholds(&[("cpu_usage", 70.0), ("memory_usage", 85.0)])
        .await;
    assert!(alerts.is_empty());

    // Test values above threshold
    let alerts = monitor
        .check_thresholds(&[("cpu_usage", 85.0), ("memory_usage", 95.0)])
        .await;
...
async fn test_error_rate_monitoring() {
    ...
    let error_rate = monitor
        .get_error_rate("api_endpoint", Duration::from_secs(60))
        .await;
...
async fn test_custom_metrics_export() {
    ...
    // Add custom metrics
    exporter.add_custom_metric(
        "business_metric",
        42.0,
        &[("environment", "test"), ("service", "worker")],
    );
