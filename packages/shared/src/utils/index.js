...
export function isValidExchange(exchange) {
...
// Trading utilities
export function calculateProfitPercentage(entryPrice, exitPrice, type) {
    if (type === 'long') {
...
export function calculatePnL(entryPrice, exitPrice, quantity, type) {
    if (type === 'long') {
...
export const extractClientId = (request) => {
...
export const extractUserIdFromAuth = (authHeader) => {
    try {
...
export const getRateLimitForRoute = (path, method, env) => {
    const routeLimits = {
        'GET:/health': 300,
...
export const createRateLimitResponse = (limit, windowStart, windowSize = 60) => {
...
export const logErrorToKV = async (errorId, error, context, kv) => {
    try {
        const errorData = {
            id: errorId,
            name: error.name,
...
export function calculateLeverage(position, collateral) {
    return position / collateral;
}
export function calculateLiquidationPrice(entryPrice, leverage, type, maintenanceMargin = 0.005) {
    if (type === 'long') {
...
export function calculateArbitrageProfit(price1, price2, quantity, fee1 = 0.001, fee2 = 0.001) {
...
export function calculatePositionSize(accountBalance, riskPercentage, entryPrice, stopLoss) {
    const riskAmount = accountBalance * (riskPercentage / 100);
    const priceRisk = Math.abs(entryPrice - stopLoss);
    return riskAmount / priceRisk;
}
export function calculateRiskReward(entryPrice, stopLoss, takeProfit, type) {
    let risk;
    let reward;
    if (type === 'long') {
...
// API utilities
export function createApiResponse(success, data, error, message) {
    return {
        success,
        data,
        error,
...
export const createDetailedErrorResponse = (error, errorId, timestamp) => {
